...
// si on clique sur on/off
onOff.onclick=function(){
//si l'interrupteur renvoie 0 (off)
  if (posOnOff===0){
    //couleur de fond noir, couleur texte blanc
lesLiens.style.backgroundColor="black";
lesLiens.style.color="white";
//pour les encadrés, filet blanc
for(let i=0; i<encadres.length-1; i++){
encadres[i].style.borderColor="white";
}
//le bouton on/off affiche (on)
onOff.textContent ="\u00A0ON\u00A0";

contenuTxt.textContent=lestextesB[motIndex];
//la valeur de l'interrupteur passe à 1 (on)
posOnOff=1;

}
//si l'interrupteur ne renvoie pas 0 (on)

else {
//on revient au choix de dépaart (off)
lesLiens.style.backgroundColor="white";
lesLiens.style.color="black";
for(let i=0; i<encadres.length-1; i++){
encadres[i].style.borderColor="black";
}
//on réaffiche (off)
...
//on passe la valeur de l'interrupteur à O (off)
posOnOff=0
}
}
